...
class RawGrouper():
    ...
    def _update_split_type_based_on_grouping(self, raw:dict[str, pd.DataFrame], grouping:dict[str, list[str]]) -> dict[str, pd.DataFrame]:
        # this dataset have no innate train/test split
        # so, leaveOneOut was used and grouping was introduced instead
        # this fn replace the split type used to mark which split is allowed,
        # to the actual split type that an example is going to
